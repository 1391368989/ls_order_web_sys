...
const apiurl="http://112.27.113.51:9040";
const proxy = {
  // 支持值为 Object 和 Array
  'GET /api/currentUser': {
    $desc: '获取当前用户接口',
  ...
  'POST /api/login/account': (req, res) => {
    ...
    if (password === '123456' && userName === 'operate') {
      res.send({
        status: 'ok',
        type,
        currentAuthority: 'operate',
      });
      return;
    }
    ...
    if (password === '123456' && userName === 'agent') {
      res.send({
        status: 'ok',
        type,
        currentAuthority: 'agent',
      });
      return;
    }
    if (password === '123456' && userName === 'dot') {
      res.send({
        status: 'ok',
        type,
        currentAuthority: 'dot',
      });
      return;
    }
    if (password === '123456' && userName === 'finance') {
      res.send({
        status: 'ok',
        type,
        currentAuthority: 'dot',
